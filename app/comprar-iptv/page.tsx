...
const pageData = KEYWORD_MAP['/comprar-iptv'];
...
export default function ComprarIptvPage() {
  const productSchema = getProductSchema();
  const faqSchema = getFaqSchema();
  const breadcrumbSchema = getBreadcrumbSchema([{ name: 'Comprar IPTV', item: pageData.route }]);

  return (
    <>
      <JsonLd data={[productSchema, faqSchema, breadcrumbSchema]} />
      <main className="mx-auto max-w-6xl px-6 pt-8">
        <Breadcrumbs items={[{ label: 'Comprar IPTV', href: pageData.route }]} />
